 ...
 **/
/**
 * Modified by Olivier Garcia (23/10/2006) : Changes : calculateHypergDistr()
 * was modified by little optimization.
 */
...
public class HyperGeometricTest {
     ...
     *
     * bigN'lik genome, bigX kadar cekmisiz, genome'daki pathwaye ait n genden,
     * x kadarini cekmisiz.
     *
     * @return String with result of calculations.
     */
    public double calculateHypergDistr(int x, int bigX, int n, int bigN) {
        ...
        /*
		 * while((bigN-n >= bigX-i)&&(i >= 0)){ BigInteger c_n_i =
		 * bi.calculateBinomialCoefficients(n, i); BigInteger c_Nminn_Xmini =
		 * bi.calculateBinomialCoefficients(bigN-n, bigX-i); BigInteger
		 * upperPartFraction = c_n_i.multiply(c_Nminn_Xmini); sum = sum.add(new
		 * BigDecimal(upperPartFraction)); i-- ; } //Substitued because, for
		 * H(bigN,smallN,bigX) : // P(X=i) == P(X=i+1) .
		 * (i+1)(bigN-smallN-bigx+i+1)/( (smallN-i)(bigX-i) )
         */
        if ((bigN - n >= bigX - (x - 1)) && (x - 1 >= 0)) {
            c_n_i = bi.calculateBinomialCoefficients(n, x - 1);
            c_Nminn_Xmini = bi.calculateBinomialCoefficients(bigN - n, bigX
                    - (x - 1));
            ...
            while ((bigN - n >= bigX - i) && (i >= 0)) {
                ...
                coeff2 = (n - i) * (bigX - i);
                upperPartFraction = upperPartFraction.multiply(new BigInteger(
                        Integer.toString(coeff1)));
                // upperPartFraction = upperPartFraction.divide(new
                // BigInteger(Integer.toString(coeff2)));
                test = upperPartFraction.divideAndRemainder(new BigInteger(
                        Integer.toString(coeff2)));
                upperPartFraction = test[0];
                if (test[1].intValue() != 0) {
                    System.out.println("ca couille ya un reste a la division "
                            + test[1]);
                    i = 0;
                }
                sum = sum.add(new BigDecimal(upperPartFraction));
        ...
        sum = sum.divide(new BigDecimal(c_N_X), SCALE_RESULT,
    ...
    // bigN'lik genome, bigX kadar �ekmi�iz, genome'daki pathwaye ait n genden,
    // x kadar�n� �ekmi�iz.
    public double calculateDirectHypergDistr(int x, int bigX, int n, int bigN) {
        if (x == n) {
            return calculateHypergDistr(x, bigX, n, bigN);
        } else {
            return (calculateHypergDistr(x, bigX, n, bigN) - calculateHypergDistr(
                    x + 1, bigX, n, bigN));
        }
